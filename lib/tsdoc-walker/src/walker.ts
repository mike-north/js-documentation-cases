...
import { isExported } from './utils/node';
import {
  reportCompilerErrors,
  dumpTSDocTree,
  sourceFilesFromEntries
} from './utils/walker';

export function advancedDemo(program: ts.Program, entries: string[]): void {
  const compilerOptions: ts.CompilerOptions = {
    target: ts.ScriptTarget.ES5
  };

  reportCompilerErrors(program.getSemanticDiagnostics());
  const sourceFiles = sourceFilesFromEntries(program, entries);
  ...
  sourceFiles.forEach(f => {
    console.log(colors.yellow(`Scanning file ${f.fileName}`));
    walkCompilerAstAndFindComments(f, '', foundComments);
  });
